...
export async function middleware(request: NextRequest) {
  ...
  if (hostname.endsWith(".ma5zani.com")) {
    ...
    // Pass through API routes and Next.js internals — they must not be rewritten
    ...
    // Skip prefixing if path already starts with /subdomain (links generated by storefront)
  ...
  // Pass through API routes and Next.js internals on custom domains too
  const customPath = request.nextUrl.pathname;
  if (customPath.startsWith("/api/") || customPath.startsWith("/_next/")) {
    return NextResponse.next();
  }
  ...
  if (cached && now < cached.expiresAt) {
    ...
    if (!(path === `/${cached.slug}` || path.startsWith(`/${cached.slug}/`))) {
      url.pathname = `/${cached.slug}${path}`;
    }
    return NextResponse.rewrite(url);
  }
  ...
  try {
    ...
    if (result && result.slug) {
      ...
      const url = request.nextUrl.clone();
      const path = url.pathname;
      if (!(path === `/${result.slug}` || path.startsWith(`/${result.slug}/`))) {
        url.pathname = `/${result.slug}${path}`;
      }
      return NextResponse.rewrite(url);
    }
  } catch (err) {
