    ...
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col h-[600px]">
      ...
      <div className="px-4 py-3 border-b border-slate-200">
        <h2 className="font-semibold text-slate-900">
          {isRTL ? 'تحدث مع المصمم' : 'Chat with Designer'}
        </h2>
        <p className="text-sm text-slate-500">
          {isRTL
            ? 'صِف متجرك والجو الذي تريده'
            : 'Describe your store and the vibe you want'}
        </p>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-semibold text-slate-900 mb-2">
                {isRTL ? 'مرحباً! أنا مصمم المتاجر الذكي' : "Hi! I'm your AI store designer"}
              </h3>
              <p className="text-slate-500 max-w-sm mx-auto">
                {isRTL
                  ? 'صِف لي نوع متجرك والألوان والأجواء التي تريدها، وسأصمم لك متجراً فريداً'
                  : 'Tell me about your store, the colors and vibe you want, and I\'ll design a unique store for you'}
              </p>
            </div>
            ...
            <div>
              <p className="text-sm font-medium text-slate-700 mb-3">
                {isRTL ? 'أفكار للبدء:' : 'Ideas to get started:'}
              </p>
            ...
            {generatedConfig && !isGenerating && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h4 className="font-semibold text-green-800 mb-2">
                  {isRTL ? 'التصميم جاهز!' : 'Design Ready!'}
                </h4>
                <div className="text-sm text-green-700 space-y-1">
                  <p>
                    <span className="font-medium">{isRTL ? 'الاتجاه:' : 'Direction:'}</span>{' '}
                    {generatedConfig.aestheticDirection}
                  </p>
                  <p>
                    <span className="font-medium">{isRTL ? 'الأقسام:' : 'Sections:'}</span>{' '}
                    {generatedConfig.sections.filter((s) => s.enabled).length}{' '}
                    {isRTL ? 'قسم' : 'sections'}
                  </p>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="font-medium">{isRTL ? 'الألوان:' : 'Colors:'}</span>
                ...
                <p className="text-sm text-green-600 mt-3">
                  {isRTL
                    ? 'شاهد المعاينة ثم اضغط "تطبيق التصميم" لحفظه'
                    : 'Check the preview then click "Apply Design" to save'}
                </p>
              </div>
            )}
      ...
      <div className="border-t border-slate-200 p-4">
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            ...
            placeholder={
              isRTL
                ? 'صِف متجرك... مثال: متجر أزياء عصرية بألوان زاهية'
                : 'Describe your store... e.g., modern fashion store with vibrant colors'
            }
            disabled={isGenerating}
        ...
        {generatedConfig && !isGenerating && (
          <div className="flex flex-wrap gap-2 mt-3">
            <span className="text-xs text-slate-500">
              {isRTL ? 'تعديلات سريعة:' : 'Quick tweaks:'}
            </span>
